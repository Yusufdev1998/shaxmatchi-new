...
export function UsersPage() {
  ...
  const createStudentMutation = useMutation({
    mutationFn: (input: { login: string; password: string }) => adminUsersApi.createStudent(input),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["adminUsers", "students"] });
    },
  });
  const updateStudentMutation = useMutation({
    mutationFn: (input: { studentId: string; login?: string; password?: string }) =>
      adminUsersApi.updateStudent(input.studentId, { login: input.login, password: input.password }),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["adminUsers", "students"] });
    },
  });
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-slate-600">O'quvchi hisoblarini yarating va boshqaring. O'quvchilar login/parol bilan kiradi.</p>
      </div>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  setError(null);
                  await createStudentMutation.mutateAsync({ login: newLogin.trim(), password: newPassword });
                  setNewLogin("");
                  setNewPassword("");
                } catch (e) {
