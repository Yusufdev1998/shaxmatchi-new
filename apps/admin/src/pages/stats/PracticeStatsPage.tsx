...
import { adminStatsApi, type PracticePuzzleStatRow } from "../../api/adminStatsApi";

function taskPuzzlesPath(row: PracticePuzzleStatRow): string {
  return `/debuts/levels/${row.levelId}/courses/${row.courseId}/modules/${row.moduleId}/tasks/${row.taskId}/puzzles`;
}

function groupRowsByStudent(rows: PracticePuzzleStatRow[]): { studentId: string; login: string; items: PracticePuzzleStatRow[] }[] {
  const map = new Map<string, PracticePuzzleStatRow[]>();
  for (const r of rows) {
...
export function PracticeStatsPage() {
  ...
  const query = useQuery({
    queryKey: ["adminStats", "practicePuzzles"],
    queryFn: adminStatsApi.listPracticePuzzles,
  });
  ...
  return (
    <div className="space-y-4">
      <AdminBreadcrumb
        compact
        items={[{ label: "Statistika", to: "/stats" }, { label: "Mashq statistikasi" }]}
      />

      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 px-4 py-3 sm:px-5">
          <h2 className="text-sm font-semibold text-slate-900">Mashq statistikasi</h2>
          <p className="mt-0.5 text-xs text-slate-500">
            O‘quvchini bosing — variantlar ochiladi. Muvaffaqiyat (yashil), xato, xato paytida o‘rtacha joylashuv, va
            qolgan urinish.
          </p>
        </div>
        ...
        <div className="p-0">
          {query.isLoading ? (
            ...
            <div className="p-4 text-sm text-red-700">
              {query.error instanceof Error ? query.error.message : "Xatolik yuz berdi"}
            </div>
          ) : rows.length === 0 ? (
            <div className="px-4 py-8 text-center text-sm text-slate-600 sm:px-5">
              Hozircha mashq tayinlovlari yo‘q.
            </div>
          ) : (
              ...
              <table className="w-full min-w-[720px] border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/80 text-xs font-medium uppercase tracking-wide text-slate-600">
                    ...
                    <th className="whitespace-nowrap px-3 py-2.5 sm:px-4">Limit</th>
                    <th className="whitespace-nowrap px-3 py-2.5 sm:px-4">Muvaffaq.</th>
                    <th className="whitespace-nowrap px-3 py-2.5 sm:px-4">Xato</th>
                    <th className="whitespace-nowrap px-3 py-2.5 sm:px-4" title="Xato qilgan paytdagi o‘rtacha progress">
                      O‘rt. xato %
                    </th>
                    <th className="whitespace-nowrap px-3 py-2.5 sm:px-4">Qoldi</th>
                          ...
                          ? items.map((row) => (
                              <tr key={row.assignmentId} className="bg-white hover:bg-slate-50/80">
                                <td
                                  ...
                                  aria-hidden
                                />
                                <td className="max-w-[200px] px-3 py-2.5 align-top sm:max-w-none sm:px-4">
                                ...
                                <td className="whitespace-nowrap px-3 py-2.5 align-top font-mono text-xs text-slate-700 sm:px-4">
                                  {row.practiceLimit === null ? "—" : row.practiceLimit}
                                </td>
                                <td className="whitespace-nowrap px-3 py-2.5 align-top font-mono text-xs text-emerald-700 sm:px-4">
                                  {row.practiceSuccessCount}
                                </td>
                                <td className="whitespace-nowrap px-3 py-2.5 align-top font-mono text-xs text-rose-700 sm:px-4">
                                  {row.practiceFailCount}
                                </td>
                                <td className="whitespace-nowrap px-3 py-2.5 align-top font-mono text-xs text-slate-600 sm:px-4">
                                  {row.averageFailProgressPercent === null
                                    ? "—"
                                    : `${row.averageFailProgressPercent}%`}
                                </td>
                                <td className="whitespace-nowrap px-3 py-2.5 align-top font-mono text-xs text-slate-700 sm:px-4">
                                  {row.practiceLeft === null ? "—" : row.practiceLeft}
                                </td>
