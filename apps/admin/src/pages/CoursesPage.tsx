...
export function CoursesPage() {
  ...
  async function remove(id: string) {
    if (!confirm("Kurs va BARCHA modullar/vazifalar/pazllarni o'chirasizmi?")) return;
    setError(null);
    try {
      ...
      <div className={debutsUi.card}>
        ...
        {loading ? (
          ...
          <div className={debutsUi.listRows}>
            {courses.map((c) => (
              <div key={c.id} className={[debutsUi.row, debutsUi.rowHover].join(" ")}>
                ...
                <div className="flex shrink-0 flex-wrap items-center justify-end gap-1">
                  <Button
                    ...
                    variant="secondary"
                    disabled={busy}
                    onClick={() => navigate(`/debuts/levels/${levelIdSafe}/courses/${c.id}/modules`)}
                  >
                    Modullar
                  </Button>
                  <button
                  ...
                  <button type="button" className={debutsUi.dangerBtn} disabled={busy} title="O'chirish" onClick={() => void remove(c.id)}>
