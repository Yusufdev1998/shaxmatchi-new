...
export type Student = { id: string; login: string; type: "student"; createdAt?: string };

export const adminUsersApi = {
  listStudents: () => api<Student[]>(`/users/students`),
  createStudent: (input: { login: string; password: string }) =>
    api<Student>(`/users/students`, { method: "POST", body: JSON.stringify(input) }),
  updateStudent: (studentId: string, input: { login?: string; password?: string }) =>
